...
export default function DiscoveryAIPanel({
  isOpen,
  onClose,
  caseData,
  discoveryType,
  ...
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      ...
      setMessages([{
        id: 'welcome',
        role: 'assistant',
        content: `I'm ready to help you draft **Special Interrogatories** for **${caseData.caseName}**.\n\n` +
          `**Case Parties:**\n` +
          `• Plaintiff: ${plaintiffName}\n` +
          `• Defendant: ${defendantName}\n\n` +
          (caseData.facts ? `**Case Facts Summary:**\n${caseData.facts.substring(0, 200)}${caseData.facts.length > 200 ? '...' : ''}\n\n` : '') +
          `I'll generate California-formatted interrogatories for the **${categoryName}** category.\n\n` +
          `**Try asking:**\n` +
          `• "Generate 5 interrogatories about how the incident occurred"\n` +
          `• "Draft questions about plaintiff's medical treatment"\n` +
          `• "Create interrogatories about damages and lost wages"\n` +
          `• "Add questions about prior injuries"\n\n` +
  ...
  const handleSend = async () => {
    ...
    try {
      const targetCategoryId = selectedCategory || categories[0]?.id || 'facts'
      ...
      setMessages(prev => [...prev, {
        id: (Date.now() + 1).toString(),
        role: 'assistant',
        content: 'I encountered an error generating interrogatories. Please try again.'
      }])
    } finally {
      setIsLoading(false)
    }
  ...
  const handleInsertSuggestions = () => {
    if (pendingSuggestions) {
      ...
      setMessages(prev => [...prev, {
        id: Date.now().toString(),
        role: 'assistant',
        content: `✅ Added ${pendingSuggestions.items.length} interrogatories to the **${categories.find(c => c.id === pendingSuggestions.categoryId)?.title}** category.\n\nWould you like me to generate more questions, or questions for a different category?`
      }])
    }
  }
  ...
  return (
    <>
      ...
      <div className="fixed right-0 top-0 h-full w-full max-w-[480px] bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 shadow-2xl flex flex-col z-50 animate-slide-in-right">
        {/* Header */}
        <div className="bg-gradient-to-r from-purple-600 to-blue-600 px-5 py-4 flex items-center justify-between shrink-0">
          <div className="flex items-center gap-3">
            <div className="p-2 bg-white/20 rounded-lg">
              <Sparkles className="h-5 w-5 text-white" />
            </div>
            <div>
              <h3 className="font-semibold text-white">Discovery AI Assistant</h3>
        ...
        <div className="px-4 py-3 bg-slate-800/50 border-b border-white/10 shrink-0">
          <label className="block text-xs text-white/60 mb-1.5">Target Category for New Items</label>
          <select
            value={selectedCategory || categories[0]?.id || ''}
            onChange={(e) => onCategorySelect(e.target.value)}
            className="w-full px-3 py-2 bg-slate-900/50 border border-white/20 rounded-lg text-white text-sm focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
          >
            {categories.map(cat => (
          ...
          <div className="flex flex-wrap gap-2">
            {quickActions.map((action, idx) => (
              <button
                ...
                className="px-3 py-1.5 text-xs bg-slate-700/50 text-white/80 border border-white/10 rounded-full hover:bg-purple-600/30 hover:border-purple-400/50 hover:text-white transition-all"
              >
                {action.label}
              </button>
            ))}
            ...
            <div className="flex justify-start">
              <div className="bg-slate-700/50 rounded-2xl px-4 py-3 border border-white/10">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-white/60">Drafting interrogatories...</span>
                </div>
              </div>
            </div>
          )}
        ...
        {/* Pending Suggestions */}
        {pendingSuggestions && (
          <div className="px-4 py-3 bg-green-900/30 border-t border-green-400/30 shrink-0">
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-medium text-green-400 flex items-center gap-2">
                <Sparkles className="w-4 h-4" />
                {pendingSuggestions.items.length} Interrogatories Ready
              </span>
              <div className="flex gap-2">
                <button
                  onClick={handleDiscardSuggestions}
                  className="text-xs px-3 py-1.5 text-white/60 hover:bg-white/10 rounded-lg transition-colors flex items-center gap-1"
                >
                ...
                <button
                  ...
                  className="text-xs px-4 py-1.5 bg-green-600 text-white rounded-lg hover:bg-green-500 font-medium flex items-center gap-1 transition-colors"
                >
            ...
            <div className="max-h-32 overflow-y-auto bg-slate-900/50 rounded-lg border border-green-400/30 p-3 text-xs text-green-100 space-y-2">
              ...
              {pendingSuggestions.items.length > 3 && (
                <p className="text-green-400 font-medium">...and {pendingSuggestions.items.length - 3} more</p>
              )}
            </div>
          </div>
        )}
        ...
        <div className="p-4 border-t border-white/10 bg-slate-800/50 shrink-0">
          <div className="flex gap-2">
            <textarea
              ...
              placeholder="Describe what interrogatories you need..."
              className="flex-1 px-4 py-3 bg-slate-900/50 border border-white/20 rounded-xl text-white placeholder-white/40 text-sm resize-none focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
              rows={2}
              disabled={isLoading}
            />
            <button
              ...
              className="self-end px-4 py-3 bg-gradient-to-r from-purple-600 to-blue-600 text-white rounded-xl disabled:opacity-50 disabled:cursor-not-allowed hover:from-purple-500 hover:to-blue-500 transition-all"
            >
