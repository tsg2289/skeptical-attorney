...
import RFACanvas from './components/RFACanvas'
      ...
      if (canAccessDatabase()) {
        ...
        if (user) {
          ...
          if (foundCase) {
            ...
            console.log(`[AUDIT] RFA accessed for case: ${caseId}`)
          }
        }
      }
      setLoading(false)
          ...
          <div className="max-w-2xl mx-auto px-4">
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-3 pt-2">
                  <button
                    ...
                    className="flex-1 bg-gradient-to-r from-amber-500 to-amber-600 text-white py-3 rounded-xl hover:from-amber-600 hover:to-amber-700 transition-all font-medium disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isEditingTrialCase ? 'Save Changes' : 'Continue to Requests for Admission'}
                  </button>
                  {isEditingTrialCase && (
                    <button
              ...
              <p className="text-xs text-gray-500 text-center mt-6">
                <Link href="/login" className="text-amber-600 hover:underline">Sign up</Link> to save your work permanently across sessions.
              </p>
            </div>
          </div>
        </section>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-blue-50">
      ...
      {isTrialMode ? (
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-amber-100">
                    {currentCase.caseNumber ? `Case #: ${currentCase.caseNumber} • ` : ''}Requests for Admission
                  </p>
                </div>
              </div>
              <button
        ...
        <div className="bg-gradient-to-r from-amber-500 to-amber-600 text-white py-4 shadow-md">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between">
              <div className="flex items-center space-x-3">
                <Link 
                ...
                <div>
                  <h2 className="text-xl font-bold">{currentCase.caseName}</h2>
                  <p className="text-sm text-amber-100">Case #: {currentCase.caseNumber} • Requests for Admission</p>
                </div>
              </div>
              <Link
                href={`/dashboard/cases/${currentCase.id}/discovery`}
                className="text-sm hover:underline text-amber-100"
              >
                Back to Discovery
              </Link>
            </div>
