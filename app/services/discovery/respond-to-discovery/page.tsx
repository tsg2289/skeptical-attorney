...
function RespondToDiscoveryPageContent() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-white">
        <Header />
        <div className="p-6 text-gray-600">Loading respond to discovery...</div>
        <Footer />
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-white">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="text-5xl md:text-6xl font-bold mb-8 leading-tight">
              <span className="gradient-text">Respond to Discovery</span>
            </h1>
            <p className="text-xl md:text-2xl text-gray-600 mb-12 leading-relaxed">
              Draft responses to discovery requests with proper objections and formatting automatically.
            </p>
          </div>
        </div>
      </section>
      ...
      ) : isTrialMode && currentCase ? (
        /* Trial mode with case data - redirect to dashboard responses page */
        <section className="py-12">
          <div className="max-w-2xl mx-auto px-4 text-center">
            <div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-8">
              <div className="text-4xl mb-4">✅</div>
              <h2 className="text-2xl font-bold text-gray-900 mb-4">Case Ready!</h2>
              <p className="text-gray-600 mb-6">
                Your case information is saved. Choose a discovery type to respond to:
              </p>
              <div className="grid grid-cols-2 gap-4">
                <Link
                  href={`/dashboard/cases/${trialCaseId}/discovery/responses?type=frog`}
                  className="p-4 rounded-xl border-2 border-violet-200 bg-violet-50 hover:bg-violet-100 transition-colors"
                >
                  <div className="font-semibold text-gray-900">Form Interrogatories</div>
                  <div className="text-sm text-gray-600">FROG</div>
                </Link>
                <Link
                  href={`/dashboard/cases/${trialCaseId}/discovery/responses?type=srog`}
                  className="p-4 rounded-xl border-2 border-purple-200 bg-purple-50 hover:bg-purple-100 transition-colors"
                >
                  <div className="font-semibold text-gray-900">Special Interrogatories</div>
                  <div className="text-sm text-gray-600">SROG</div>
                </Link>
                <Link
                  href={`/dashboard/cases/${trialCaseId}/discovery/responses?type=rfp`}
                  className="p-4 rounded-xl border-2 border-rose-200 bg-rose-50 hover:bg-rose-100 transition-colors"
                >
                  <div className="font-semibold text-gray-900">Requests for Production</div>
                  <div className="text-sm text-gray-600">RFP</div>
                </Link>
                <Link
                  href={`/dashboard/cases/${trialCaseId}/discovery/responses?type=rfa`}
                  className="p-4 rounded-xl border-2 border-fuchsia-200 bg-fuchsia-50 hover:bg-fuchsia-100 transition-colors"
                >
                  <div className="font-semibold text-gray-900">Requests for Admission</div>
                  <div className="text-sm text-gray-600">RFA</div>
                </Link>
              </div>
              <Link
                href={`/dashboard/cases/${trialCaseId}/discovery`}
                className="inline-block mt-6 text-blue-600 hover:underline"
              >
                Or go to Discovery Hub →
              </Link>
            </div>
          </div>
        </section>
      ) : (
        /* Coming Soon Section - for non-trial users without a case */
            ...
            <div className="text-center max-w-2xl mx-auto">
              <div className="bg-white rounded-3xl shadow-xl border border-gray-100 p-12">
                ...
                <p className="text-xl text-gray-600 leading-relaxed">
                  We&apos;re working hard to bring you the Respond to Discovery tool. Check back soon!
                </p>
              </div>
            </div>
          </div>
