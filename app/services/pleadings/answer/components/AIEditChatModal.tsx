...
export default function AIEditChatModal({
  ...
  // Answer-specific quick actions
  const quickActions = [
    { label: 'Strengthen defense', prompt: 'Make this defense more robust with stronger legal language and additional supporting arguments' },
    { label: 'Add legal citations', prompt: 'Add relevant California statutory citations and case law references to strengthen this defense' },
    { label: 'More specific facts', prompt: 'Make this defense more fact-specific based on the complaint allegations' },
    { label: 'More concise', prompt: 'Make this defense more concise while keeping the key legal elements' },
    { label: 'Add alternative theory', prompt: 'Suggest an alternative legal theory or additional affirmative defense that could apply' },
  ];

  // Initialize with greeting when modal opens
  useEffect(() => {
    if (isOpen && messages.length === 0) {
      const defendantWord = isMultipleDefendants ? 'Defendants' : 'Defendant';
      const plaintiffNames = parties?.plaintiffs?.map(p => p.name).join(', ') || 'Plaintiff';
      const defendantNames = parties?.defendants?.map(d => d.name).join(', ') || 'Defendant';
      
      setMessages([
        {
          id: 'welcome',
          role: 'assistant',
          content: `Hello! I'm here to help you edit the **${sectionTitle}** section of your Answer to Complaint.\n\n**Case Parties:**\n• Plaintiff(s): ${plaintiffNames}\n• Defendant(s): ${defendantNames}\n\nI can help you:\n• Strengthen your affirmative defenses\n• Add California statutory citations\n• Make defenses more fact-specific\n• Suggest additional defenses based on the complaint\n\nTell me what changes you'd like, and I'll help you refine this section while maintaining proper ${defendantWord} (${isMultipleDefendants ? 'plural' : 'singular'}) grammar throughout.`,
          timestamp: new Date(),
        },
      ]);
    }
  }, [isOpen, sectionTitle, messages.length, isMultipleDefendants, parties]);
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {quickActions.map((action, idx) => (
              <button
                ...
                className="px-3 py-1.5 text-xs font-medium bg-gray-200 text-gray-700 border border-gray-300 rounded-full hover:bg-primary-100 hover:border-primary-400 hover:text-primary-700 transition-colors whitespace-nowrap"
              >
                {action.label}
              </button>
            ))}
        ...
        <div className="p-4 border-t bg-white shrink-0">
          <div className="flex gap-2">
            <textarea
              ...
              placeholder="Tell me how you'd like this defense edited..."
              className="flex-1 resize-none border border-gray-300 rounded-xl px-4 py-3 focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm text-gray-900 bg-gray-50 placeholder-gray-500"
              rows={2}
              disabled={isLoading}
            />
            <button
              ...
              className="self-end px-4 py-3 bg-primary-600 text-white rounded-xl hover:bg-primary-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
