...
export default function AIEditChatModal({
  ...
  // Settlement agreement-specific quick actions
  const quickActions = [
    { label: 'Strengthen release', prompt: 'Make this release section more comprehensive and protective, ensuring broader coverage of potential claims' },
    { label: 'More favorable terms', prompt: 'Adjust the language to be more favorable to the releasing party while maintaining enforceability' },
    { label: 'Add specificity', prompt: 'Add more specific detail and enumerate covered claims more thoroughly' },
    { label: 'Simplify language', prompt: 'Simplify this section while maintaining legal effectiveness and enforceability' },
    { label: 'Make mutual', prompt: 'Make this section mutual/bilateral where appropriate' },
  ];

  // Get section-specific suggestions
  const getSectionType = (title: string): string => {
    const lowerTitle = title.toLowerCase();
    if (lowerTitle.includes('preamble') || lowerTitle.includes('recital')) return 'preamble';
    if (lowerTitle.includes('consideration') || lowerTitle.includes('payment')) return 'consideration';
    if (lowerTitle.includes('release') || lowerTitle.includes('general release')) return 'release';
    if (lowerTitle.includes('tax') || lowerTitle.includes('responsibility')) return 'tax';
    if (lowerTitle.includes('confidential') || lowerTitle.includes('non-disclosure')) return 'confidentiality';
    if (lowerTitle.includes('indemnif')) return 'indemnification';
    if (lowerTitle.includes('dismissal')) return 'dismissal';
    if (lowerTitle.includes('signature')) return 'signature';
    if (lowerTitle.includes('1542') || lowerTitle.includes('unknown')) return 'unknown_claims';
    return 'general';
  };
  ...
  const getSectionSpecificSuggestions = (type: string): string => {
    switch (type) {
      case 'preamble':
        return 'For the preamble and recitals, I can help you:\n• Clarify party identification\n• Strengthen the background context\n• Add relevant litigation history';
      case 'consideration':
        return 'For the consideration section, I can help you:\n• Clarify payment terms and timing\n• Add tax allocation language\n• Specify payment conditions';
      case 'release':
        return 'For the general release, I can help you:\n• Broaden the scope of released claims\n• Add specific claim categories\n• Strengthen the release language\n• Ensure California compliance';
      case 'tax':
        return 'For tax responsibility provisions, I can help you:\n• Clarify tax obligations\n• Strengthen indemnification for tax liability\n• Add IRS form requirements';
      case 'confidentiality':
        return 'For confidentiality provisions, I can help you:\n• Define permitted disclosures\n• Add exceptions for legal requirements\n• Strengthen breach remedies';
      case 'indemnification':
        return 'For indemnification clauses, I can help you:\n• Expand scope of indemnification\n• Add specific indemnified claims\n• Clarify indemnification procedures';
      case 'dismissal':
        return 'For dismissal provisions, I can help you:\n• Specify dismissal timing and procedures\n• Add prejudice/without prejudice language\n• Include cost allocation';
      case 'unknown_claims':
        return 'For the 1542 waiver, I can help you:\n• Strengthen the waiver language\n• Add acknowledgment provisions\n• Ensure proper California Civil Code compliance';
      default:
      ...
      setMessages([
        {
          id: 'welcome',
          role: 'assistant',
          content: `Hello! I'm here to help you edit the **${sectionTitle}** section of your settlement agreement.\n\n${getSectionSpecificSuggestions(sectionType)}\n\nTell me what changes you'd like, and I'll help you refine this section while maintaining legal enforceability.`,
          timestamp: new Date(),
        },
      ]);
    }
