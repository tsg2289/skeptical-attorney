...
function StatusReportPageContent() {
  ...
  const [sections, setSections] = useState<CardSection[]>([
    ...
    { 
      ...
      content: '[This section will be auto-generated by AI]\n\nThis matter stems from [FULL STATEMENT OF THE CASE FACTS INCLUDING DATE, ADDRESS WITH PLAINTIFF\'S ALLEGATIONS, INJURIES, DAMAGES, ETC.]\n\nThe AI will draft this section as a competent attorney, laying out all facts of the case based solely on the information provided. No hallucinations - only verified case data will be used.' 
    },
    { 
      id: '2', 
      title: 'PROCEDURAL STATUS', 
      content: '[This section will be auto-generated by AI]\n\nA flowing paragraph covering venue, trial status, mediation/MSC status, and settlement discussions - written as a cohesive narrative without section headers.' 
    },
    { 
      id: '3', 
      title: 'PLEADINGS', 
      content: '[This section will be auto-generated by AI]\n\nPLAINTIFF\'S COMPLAINT\n• Defendants named and Does 1-100\n• Each cause of action with CACI elements and specific allegations\n\nDEFENDANT\'S ANSWER\n• Answer status (filed or pending)\n• Cross-complaint plans (indemnity/contribution)\n• Demurrer or Motion to Strike plans\n\nOTHER PLEADINGS\n• Amendments, dismissed claims, Doe amendments' 
    },
    { 
      id: '4', 
    ...
    { 
      id: '6', 
      title: 'LIABILITY', 
      content: '[This section will be auto-generated by AI]\n\nThis is the most detailed section - each cause of action as a header with well-written narrative paragraphs analyzing elements, applying facts, and assessing likelihood of prevailing. Includes comparative fault analysis and overall liability assessment.' 
    },
    { 
      id: '7', 
      title: 'DAMAGES', 
  ...
  // Reset to start a new report - keeps case summary populated
  const handleNewReport = () => {
    // Keep the case facts in Case Summary if available
    const caseSummaryContent = currentCase?.facts || 
      'Provide a comprehensive summary of the case including key facts, parties involved, claims, and current status. This will be used to generate the status report sections.';
    ...
    // Keep case name and number, only reset other fields
    setReportInfo(prev => ({
      ...prev,
      reportDate: new Date().toISOString().split('T')[0],
  ...
  const handleAIAssist = async (sectionId: string) => {
    ...
    try {
      ...
      if (selectedDocumentIds.length > 0 && currentCaseId) {
        try {
      ...
      const requestBody = { 
        ...
        // Include case data for populating procedural status fields
        caseData: currentCase ? {
          court: currentCase.court,
          courtCounty: currentCase.courtCounty,
          trialDate: currentCase.trialDate,
          mscDate: currentCase.mscDate,
          judgeName: currentCase.judgeName,
          departmentNumber: currentCase.departmentNumber,
          caseNumber: currentCase.caseNumber,
          caseName: currentCase.caseName,
          complaintFiledDate: currentCase.complaintFiledDate,
      ...
      if (data.sections) {
        console.log('AI returned sections:', Object.keys(data.sections));
        setSections(prevSections => 
          prevSections.map(sec => {
            // Handle both string and numeric keys from AI response
            const content = data.sections[sec.id] || data.sections[Number(sec.id)] || data.sections[String(sec.id)];
            if (content) {
              console.log(`Updated section ${sec.id}: ${sec.title}`);
              return { ...sec, content };
            }
            console.log(`No content for section ${sec.id}: ${sec.title}`);
            return sec;
          })
        );
        ...
        console.error('No sections in API response:', data);
      }
    } catch (error) {
              ...
              {showDocumentSelector && (
                <div className="mt-3 p-4 bg-gray-50 rounded-xl border border-gray-200">
                  <p className="text-sm text-gray-600 mb-3">
                    Select documents to include their content when generating the status report. 
                    The AI will extract facts, dates, medical information, and procedural details from these documents.
                  </p>
                  {loadingDocuments ? (
            ...
            >
              {aiLoading === '0' ? (
                ...
                <>
                  ...
                  <span>Generate AI-Powered Status Report{selectedDocumentIds.length > 0 ? ` (with ${selectedDocumentIds.length} docs)` : ''}</span>
                </>
              )}
            </button>
          )}
